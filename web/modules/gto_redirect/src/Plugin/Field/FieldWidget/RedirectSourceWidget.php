...
namespace Drupal\gto_redirect\Plugin\Field\FieldWidget;
		...
		if ($items->getEntity()->isNew()) {
			...
			if ($source_path) {
				...
				try {
					...
					$element['status_box'][]['#markup'] = '<div class="messages messages--warning">' . $this->t(
						...
						['%path' => $source_path, '@url-alias' => Url::fromRoute('entity.path_alias.add_form')->toString()]
					) . '</div>';
				...
				if (!empty($path)) {
					...
					$repository = \Drupal::service('gto_redirect.repository');
